...
static int __wchar_forbitten(av_wchar_t sym);
static int __utf8_forbitten(u_char octet);

static int
__wchar_forbitten(av_wchar_t sym)
{
 ...
 */
size_t
utf8_to_wchar(const char *in, size_t insize, av_wchar_t *out, size_t outsize,
    int flags)
{
	u_char *p, *lim;
	av_wchar_t *wlim, high;
	...
	for (; p < lim; p += n) {
		...
		if ((*p & 0x80) == 0)
			...
			n = 2;
			high = (av_wchar_t)(*p & 0x1f);
		} else if ((*p & 0xf0) == _SEQ3) {
			n = 3;
			high = (av_wchar_t)(*p & 0x0f);
		} else if ((*p & 0xf8) == _SEQ4) {
			n = 4;
			high = (av_wchar_t)(*p & 0x07);
		} else if ((*p & 0xfc) == _SEQ5) {
			n = 5;
			high = (av_wchar_t)(*p & 0x03);
		} else if ((*p & 0xfe) == _SEQ6) {
			n = 6;
			high = (av_wchar_t)(*p & 0x01);
		} else {
		...
		*out = 0;
		n_bits = 0;
		for (i = 1; i < n; i++) {
			*out |= (av_wchar_t)(p[n - i] & 0x3f) << n_bits;
 ...
 */
size_t
wchar_to_utf8(const av_wchar_t *in, size_t insize, char *out, size_t outsize,
    int flags)
{
	av_wchar_t *w, *wlim, ch;
	u_char *p, *lim, *oc;
	size_t total, n;
